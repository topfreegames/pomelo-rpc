...
pro.listen = function(port) {
  ...
  this.server.on('connection', function(socket) {
    ...
    socket._handshakeTimeout = setTimeout(function() {
      if (socket._handshakeDone) return;
      socket._handshakeDone = true;
      socket.removeListener('data', onData);
      var buf = Buffer.concat(handshakeBuf);
      // On timeout, try to parse once in case we have handshake + remainder (e.g. two TCP segments).
      // If not valid handshake (e.g. legacy client sent Composer binary), parsed is null → same as before.
      var parsed = handshake.parseClientHandshake(buf);
      socket._handshakeTimeout = null;
      // If socket was already closed (e.g. client disconnected), close handler removed it; do not set up.
      if (!self.sockets[socket.id] || self.sockets[socket.id] !== socket) return;
      if (parsed) {
        ...
        socket.write(handshake.serializeServerHandshake(chosen));
    ...
    var onData = function(data) {
      ...
      if (parsed) {
        socket._handshakeDone = true;
        if (socket._handshakeTimeout) { clearTimeout(socket._handshakeTimeout); socket._handshakeTimeout = null; }
        socket.removeListener('data', onData);
        var chosen = handshake.negotiateCompression(serverSupported, parsed.msg.compression);
        chosen = handshake.resolveCompression(chosen, parsed.msg.compression, compression.getCompressor);
    ...
    socket.on('close', function() {
      ...
      if (socket.composer) socket.composer.removeAllListeners();
      if (socket._compressStream && typeof socket._compressStream.destroy === 'function') socket._compressStream.destroy();
      if (socket._decompressStream && typeof socket._decompressStream.destroy === 'function') socket._decompressStream.destroy();
...
function finishServerHandshake(socket, acceptor, chosenCompression, remainder, oldDataListener) {
  ...
  if (remainder && remainder.length > 0) {
    // Remainder can contain compressed data when the first read delivered handshake + first frame
    // (e.g. TCP coalescing, or client sent first RPC right after handshake in same tick).
    // When compression is negotiated, feed remainder through the decompress stream.
    if (socket._decompressStream) {
      socket._decompressStream.write(remainder);
    } else {
...
pro.close = function() {
  ...
  try {
    ...
    for (var socketId in this.sockets) {
      ...
      var s = this.sockets[socketId];
      if (s) {
        if (s.composer) s.composer.removeAllListeners();
        if (s._compressStream && typeof s._compressStream.destroy === 'function') s._compressStream.destroy();
        if (s._decompressStream && typeof s._decompressStream.destroy === 'function') s._decompressStream.destroy();
        s.end();
      }
