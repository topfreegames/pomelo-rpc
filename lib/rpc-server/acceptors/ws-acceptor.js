...
pro.listen = function(port) {
  ...
  this.server.on('connection', function(socket) {
    ...
    socket.on('message', function(pkg) {
      try {
        if (Buffer.isBuffer(pkg)) pkg = pkg.toString();
        pkg = JSON.parse(pkg);
...
var processMsg = function(socket, acceptor, pkg) {
  var peerHost = socket.remoteAddress || (socket._socket && socket._socket.remoteAddress);
  var peerPort = socket.remotePort || (socket._socket && socket._socket.remotePort);
  ...
  var cb = acceptor.cb.bind(null, tracer, pkg.msg, function() {
    serverSpan.finish(arguments[0] instanceof Error ? arguments[0] : null);
