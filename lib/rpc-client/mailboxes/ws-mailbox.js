 ...
 */
 pro.send = function(tracer, msg, opts, cb) {
  ...
  if (tracer.span && tracer.span.inject && !tracer.span.isNoop) {
    ...
    var peerHost = this.socket && (this.socket.remoteAddress || (this.socket._socket && this.socket._socket.remoteAddress));
    var peerPort = this.socket && (this.socket.remotePort || (this.socket._socket && this.socket._socket.remotePort));
    if (peerHost || peerPort) {
      tracer.span.addTags({ 'peer.host': peerHost, 'peer.port': peerPort });
    }
  }
  if(this.bufferMsg) {
    enqueue(this, pkg);
