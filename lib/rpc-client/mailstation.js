...
pro.stop = function(force) {
  ...
  function closeAll() {
    ...
    for(var id in self.mailboxesClosingMap) {
      self.mailboxesClosingMap[id].close();
    }
  }
  if(force) {
    closeAll();
...
pro.dispatch = function(tracer, serverId, msg, opts, cb) {
  ...
  if(!mailbox) {
    ...
    // try to connect remote server if mailbox instance does not exist yet
  ...
  var send = function(tracer, err, serverId, msg, opts) {
    ...
    mailbox.send(tracer, msg, opts, function() {
      ...
      if(!!send_err) {
        logger.error('[pomelo-rpc] fail to send message to server ' + serverId + ' with error ' + send_err);
...
pro.connect = function(tracer, serverId, cb) {
  ...
  mailbox.connect(tracer, function(err) {
    ...
    mailbox.on('close', function(id) {
      ...
      mbox = self.mailboxesClosingMap[id]
      if(!!mbox) {
        delete self.mailboxesClosingMap[id]
        mbox.close();
      }
      self.emit('close', id);
    });
    mailbox.on('closing', function(id) {
      logger.info('mailbox closing with id: %s', id);
      var mbox = self.mailboxesMap[id];
      if(!!mbox) {
        self.mailboxesClosingMap[id] = mbox;
        delete self.mailboxesMap[id];
      }
    });
